    ...
    <div class={{ [className]: Boolean(className), [styles.transactionValue]: true }}>
      ...
      {value.showFormattedAmount && (
        <div class={styles.transactionValueAmount}>
          {value.rewaLabel && <drt-dharitri-symbol-icon class={styles.transactionValueAmountSymbol} />}
      ...
      {value.link && (
        ...
        >
          <div class={styles.transactionValue}>
            {value.svgUrl && <img src={value.svgUrl} alt={value.name ?? ''} class={styles.transactionValueImg} />}

            {value.linkText && (
              <span
                class={{
                  [styles.transactionValueTextTruncate]: value.ticker === value.collection && value.ticker != null,
                }}
              >
                {value.linkText}
              </span>
      ...
      {value.titleText && (
        <drt-tooltip trigger={<Icon name="layers" class={styles.transactionValueIcon} />}>
          {value.titleText}
        </drt-tooltip>
      )}
    </div>
