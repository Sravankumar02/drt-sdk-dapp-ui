...
describe('TransactionAccount tests', () => {
  ...
  it('should render basic account information', async () => {
    ...
    expect(page.root).toEqualHtml(`
      <div class="transaction-account drt:flex drt:items-center drt:gap-2">
        <div class="transaction-account-name drt:w-max drt:truncate" title="Test Account">
          John Doe
        </div>
      </div>
    `);
  ...
  it('should render explorer link when showLink is true', async () => {
    ...
    expect(page.root).toEqualHtml(`
      <div class="transaction-account drt:flex drt:items-center drt:gap-2">
        <a class="explorer-link drt:decoration-0 drt:flex drt:text-primary! transaction-account-explorer-link" data-testid="receiverLink" href="https://example.com/account/0x1234567890123456789012345678901234567890" rel="noreferrer" target="_blank">
          <span>
  ...
  it('should apply custom class and data-testid', async () => {
    ...
    expect(page.root).toEqualHtml(`
      <div class="transaction-account drt:flex drt:items-center drt:gap-2 custom-class" data-testid="transaction-account">
        <div class="transaction-account-name drt:w-max drt:truncate" title="Test Account">
          John Doe
        </div>
      </div>
